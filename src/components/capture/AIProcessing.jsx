  ...
  useEffect(() => {
    ...
    const processImage = async () => {
      ...
      try {
        ...
        const response = await fetch(`${import.meta.env.VITE_API_URL ?? (['localhost', '127.0.0.1'].includes(window.location.hostname) ? `http://${window.location.hostname}:8000` : '')}/api/capture/analyze`, {
          method: 'POST',
          body: formData
        });
