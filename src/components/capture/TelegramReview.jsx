...
export default function TelegramReview() {
  ...
  const submitAccepted = async () => {
    ...
    try {
      ...
      const res = await fetch(`${import.meta.env.VITE_API_URL ?? (['localhost', '127.0.0.1'].includes(window.location.hostname) ? `http://${window.location.hostname}:8000` : '')}/api/telegram/accept`, {
        method: 'POST',
